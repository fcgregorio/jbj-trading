...
router.post('/logout', loginRequiredMiddleware, async function (req: Request, res: Response, next: NextFunction) {
    try {
        const result = await sequelize.transaction(async (t) => {
            const token = await AuthToken.findByPk(
                res.locals.token,
                {
                    rejectOnEmpty: true,
                    transaction: t,
...
router.post('/generate-password', async function (req: Request, res: Response, next: NextFunction) {
    try {
