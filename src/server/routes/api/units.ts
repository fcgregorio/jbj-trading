...
import sequelize, { Unit, UnitAttributes, UnitHistory, UnitHistoryAttributes, User } from '../../sequelize';
...
router.get('/', async function (req: Request, res: Response, next: NextFunction) {
    try {
        ...
        let where: WhereOptions<UnitAttributes> = {};
        ...
        if (cursorQuery !== undefined) {
            const cursor = await Unit.findByPk(
                cursorQuery,
                {
...
router.get('/:id', async function (req: Request, res: Response, next: NextFunction) {
    try {
        ...
        const result = await Unit.findByPk(
            req.params.id,
            {
                rejectOnEmpty: true,
                paranoid: paranoid,
...
router.get('/:id/histories', adminRequiredMiddleware, async function (req: Request, res: Response, next: NextFunction) {
    try {
        let where: WhereOptions<UnitHistoryAttributes> = {
            id: req.params.id,
        };
        ...
        const cursorQuery = req.query.cursor as string;
        if (cursorQuery !== undefined) {
            const cursor = await UnitHistory.findByPk(
                cursorQuery,
                {
...
router.delete('/:id', adminRequiredMiddleware, async function (req: Request, res: Response, next: NextFunction) {
    try {
        const result = await sequelize.transaction(async (t) => {
            const unit = await Unit.findByPk(
                req.params.id,
                {
                    rejectOnEmpty: true,
                    transaction: t,
