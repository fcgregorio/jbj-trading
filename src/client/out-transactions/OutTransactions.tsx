...
import { isSafeInteger, throttle } from "lodash";
...
export function OutTransferStrip(props: OutTransferStripProps) {
  ...
  React.useEffect(() => {
    const outTransferIDs = props.outTransfers.map(
      (outTransfer) => outTransfer.item
    );
      ...
      (results: Item[]) => {
        ...
        if (results) {
          newOptions = results.filter(
            (item) => !outTransferIDs.includes(item.id!)
          );
        }
...
export default function OutTransaction() {
  ...
  return (
    <Routes>
      ...
      <Route path=":outTransactionID" element={<Show />} />
      <Route
