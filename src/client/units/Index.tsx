...
export default function Index() {
    ...
    () =>
      debounce(
        async (
          ...
          startCallback: () => void,
          callback: (results: { count: number; results: Unit[] }) => void,
          errorCallback: () => void,
  ...
  }, [queryUnits, refresh, search, filterMenuShowDeleted, order]);
  ...
  async function handleLoadMoreClick() {
    ...
    await axios
      .get<{ count: number; results: Unit[] }>(`/api${location.pathname}`, {
        params: {
          search: search,
          filters: {
            showDeleted: filterMenuShowDeleted,
      ...
      .then((data) => {
        setCount(data.count);
        const newUnits = [...units, ...data.results];
        setUnits(newUnits);
        if (newUnits.length === data.count) {
          setCursor(null);
        } else if (data.results.length !== 0) {
          setCursor(data.results[data.results.length - 1].id);
        } else {
